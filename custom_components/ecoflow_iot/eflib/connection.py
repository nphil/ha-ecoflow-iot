...
class Connection:
    ...
    # MODIFICATION vs upstream (ha-ecoflow-iot): explicit contract check for the
    # account id. `DeviceBase.connect` typed it `str | None` while this class requires
    # `str`, and a missing value used to surface from the logging layer instead - the
    # connection logger builds its redaction pattern with `re.compile(user_id)` and
    # raised "first argument must be string or compiled pattern". Authentication needs
    # it for every session-key scheme: `_auto_authentication` runs unconditionally and
    # always sends MD5(user_id + serial). Exposed as a staticmethod so the reuse path
    # in `DeviceBase.connect` - which assigns `_user_id` on a live Connection without
    # going through `__init__` - is covered by the same check.
    @staticmethod
    def validate_user_id(user_id: str) -> str:
        if not isinstance(user_id, str) or not user_id:
            raise ValueError(
                "user_id is required to authenticate with an EcoFlow device "
                f"(got {user_id!r}); it is part of the auth secret for every "
                "encryption type"
            )
        return user_id
