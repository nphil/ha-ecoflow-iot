...
class Device(DeviceBase, RawDataProps):
    ...
    # MODIFICATION vs upstream (ha-ecoflow-iot): upstream exposes this as the instance
    # `device` property; it is a classmethod here so a discovery can be named without
    # constructing a device, exactly as for `river3.Device` and `wave3.Device` (see
    # `DeviceBase.model_name`). `river2_pro.Device` inherits it unchanged - its own
    # `SN_PREFIX` narrows `check()` to R621/R623, and this match already resolves those
    # to "River 2 Pro".
    @classmethod
