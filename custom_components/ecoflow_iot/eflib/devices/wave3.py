...
pb_runtime = proto_attr_mapper(ac517_apl_comm_pb2.RuntimePropertyUpload)
...
class Device(DeviceBase, ProtobufProps):
    ...
    # MODIFICATION vs upstream (ha-ecoflow-iot): `data_parse` below already decodes
    # RuntimePropertyUpload (cmd_id 0x16), but upstream declares no field for its
    # temperatures, so every one of them was parsed and dropped. All five carry
    # explicit presence in ac517_apl_comm.proto, so a unit that does not report one
    # simply leaves it None. Like every other Wave 3 temperature these are in the
    # unit selected on the device - see `temp_unit`.
    temp_indoor_return_air = pb_field(pb_runtime.temp_indoor_return_air, pround(1))
    temp_outdoor_ambient = pb_field(pb_runtime.temp_outdoor_ambient, pround(1))
    temp_condenser = pb_field(pb_runtime.temp_condenser, pround(1))
    temp_evaporator = pb_field(pb_runtime.temp_evaporator, pround(1))
    temp_compressor_discharge = pb_field(
        pb_runtime.temp_compressor_discharge, pround(1)
    )
    temp_unit = pb_field(pb_disp.user_temp_unit, TemperatureUnit.from_mode)

    # MODIFICATION vs upstream (ha-ecoflow-iot): readback fields for the three write
    # commands added at the bottom of this class. Each one is the `cfg_`-less twin of
    # a ConfigWrite field that ac517_apl_comm.proto already declares, so the control
    # reflects what the device accepted instead of an optimistic guess.
    ac_charging_speed = pb_field(pb_disp.plug_in_info_ac_in_chg_pow_max)
    max_ac_charging_power = pb_field(pb_disp.plug_in_info_ac_in_chg_hal_pow_max)
    ...
    # MODIFICATION vs upstream (ha-ecoflow-iot): these two setters are undecorated
    # upstream, so `get_controls` cannot discover them and the HA layer has to carry a
    # parallel table to build the two number entities. Decorated here exactly as
    # river3.py does, so control discovery has a single source. `limit` widened from
    # `int` to `float` to match river3 (HA number entities deliver floats) with the
    # same int() coercion at the wire.
    @controls.battery(
    ...
    async def set_battery_charge_limit_min(self, limit: float):
        ...
        await self._send_config_packet(
            ac517_apl_comm_pb2.ConfigWrite(cfg_min_dsg_soc=int(limit))
        )
        return True
    ...
    async def set_battery_charge_limit_max(self, limit: float):
        ...
        await self._send_config_packet(
            ac517_apl_comm_pb2.ConfigWrite(cfg_max_chg_soc=int(limit))
        )
        return True
    ...
    # MODIFICATION vs upstream (ha-ecoflow-iot): three write commands upstream leaves
    # unwired. Only ConfigWrite fields whose semantics are fully pinned by the
    # vendored Apache-2.0 sources are implemented here - see the NOTICE for the ones
    # deliberately left out and what evidence each is missing.
    #
    # `cfg_user_temp_unit` is typed `USER_TEMP_UNIT_TYPE` in ac517_apl_comm.proto with
    # named values (NONE/C/F), it reads back as `user_temp_unit`, and upstream already
    # ships `TemperatureUnit.as_pb_enum()` for exactly this write - dead code until
    # now. NOTE: this switches the unit of every reported temperature AND of the
    # setpoints, so a consumer must resolve units from `temp_unit`, never assume C.
    @controls.select(temp_unit, options=TemperatureUnit, exclude=[TemperatureUnit.NONE])
    async def set_temp_unit(self, unit: TemperatureUnit):
        await self._send_config_packet(
            ac517_apl_comm_pb2.ConfigWrite(cfg_user_temp_unit=unit.as_pb_enum())
        )

    # `cfg_lcd_show_temp_type` selects which temperature the panel shows. The value
    # set is upstream's own `TemperatureDisplayType` (AMBIENT=0, SUPPLY_AIR=1), also
    # dead code until now, and it reads back as `lcd_show_temp_type`.
    @controls.select(lcd_show_temp_type, options=TemperatureDisplayType)
    async def set_lcd_show_temp_type(self, display_type: TemperatureDisplayType):
        await self._send_config_packet(
            ac517_apl_comm_pb2.ConfigWrite(cfg_lcd_show_temp_type=display_type.value)
        )

    # `cfg_plug_in_info_ac_in_chg_pow_max` caps AC charge power in watts. Identical
    # shape to river3's implemented control: the current value reads back as
    # `plug_in_info_ac_in_chg_pow_max` and the device publishes its own ceiling as
    # `plug_in_info_ac_in_chg_hal_pow_max`, so no bound has to be invented.
